...
def phss_get_images(slides, ndx, token):
	files = []
	while True:
		if ndx < len(slides):
			if slides[ndx].shapes[0].has_text_frame:
				if slides[ndx].shapes[0].text_frame.text.find(token) != -1:
					files.append(ndx)
					ndx = ndx + 1
					continue
		break
...
#				espcopy = "Traducción " + esp['translation_copyright']
				espcopy = "Traducci\xc3\xb3n " + esp['translation_copyright']
		if meta['copyright'] != '':
			add_text_run(v.text_frame, meta['copyright'], 'Avenir Next LT Pro', 8, espcopy is not None, bold=False)
		v.text_frame.margin_top = 0
		v.text_frame.margin_bottom = 0
		if espcopy:
			add_text_run(v.text_frame, espcopy.encode("utf-8"), 'Alegreya Sans Medium', 9, bold=False)
...
#				filename = basename + "-" + f"{ndx:02d}" + ".png"
				...
				box = (0, 0, img.size[0], img.size[1] - 49)
