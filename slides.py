...
vex_esp = dict(omit="Omitir el verso ", omits="Omitir los versos ", chorusafterone="Coro despu�s del verso ", chorusaftermany="Coro despu�s de los versos ", chorusafterlast="Coro despu�s del �ltimo verso", nochorus="Omitir el coro")
...
def add_verse_to_deck_tall(slide, displayBook, displaySong, basename, ndx, meta, esp, navinfo, bg_fn, invert, cdh):
	...
	if esp is None:
		...
		if 'translation_copyright' in esp:
			...
			width = 1.375
			set_placeholder_size(v, top, left, width, v.height.inches)
			v.text = "Traducci�n " + esp['translation_copyright']
		else:
			v._element.getparent().remove(v._element)
...
def add_verse_to_deck_wide(slide, displayBook, displaySong, basename, ndx, meta, esp, navinfo, bg_fn, invert):
	...
	if meta['copyright'] == '' and esp is None:
		...
		if esp:
			if 'translation_copyright' in esp:
				espcopy = "Traducci�n " + esp['translation_copyright']
		if meta['copyright'] != '':
...
def add_song(outp, language, item, navitems, ndi, title=True, music=True):
	...
	if item['esp'] != None:
		...
		if bubble != None:
			item['esp']['bubble'] = "Himno de Invitaci�n"
...
#		eng[LAYOUT_TITLE_CREDITS] = dict(text='\n'.join(meta['credits'].splitlines()), max_size=12, step_size=2)
	...
	if language == "bil" or language == "esp":
		if 't�tulo' in item:
			...
			esp[LAYOUT_TITLE_DETAIL] = dict(text=item['t�tulo'].upper(), max_size=56, step_size=6, bold=True, size=[1.62, 3.75, 4.2, 1.8])
			esp[LAYOUT_TITLE_QUOTE] = dict(text=u"\u201CCANTANDO...AL SE�OR\u201D\nCOLOSENSES 3:16", max_size=22, step_size=2, size=[4.280, 4.1, 3.5, 0.9])
#			esp[LAYOUT_TITLE_CREDITS] = dict(text='\n'.join(espm['credits'].splitlines()), max_size=13, step_size=1)
...
def add_ls(outp, language, item, navitems, ndi):
	...
	if dointro:
		...
		if language == "bil" or language == "esp":
			...
			esp[LAYOUT_TITLE_REFERENCE] = dict(text="SE�OR", max_size=72, step_size=6, bold=True, size=[2.575, 3.35, 5.0, 2.4])
			esp[LAYOUT_TITLE_QUOTE] = dict(text=u"\u201CHACED ESTO EN MEMORIA DE M�\u201D LUCAS 22:19", max_size=24, step_size=2, size=[3.925, 3.67, 4.36, 1.0])
...
def add_invitation(outp, language, item, navitems, ndi):
	...
	if language == "bil" or language == "esp":
		esp = dict()
		esp[LAYOUT_TITLE_DETAIL] = dict(text="INVITACI�N", max_size=68, step_size=6, bold=True, size=[1.59, 3.35, 5.0, 1.8])
		esp[LAYOUT_TITLE_QUOTE] = dict(text=u"\u201CVENID A M�...Y YO OS HAR� DESCANSAR\u201D", max_size=26, step_size=2, size=[3.29, 3.47, 4.78, 1.0])
...
def parse_worship_item(order, item, language):
	...
	if item['type'] == 'song' or item['type'] == 'song-music':
		...
		if (language == 'esp') and ('t�tulo' in item):
			title = item['t�tulo']
		else:
			title = item['title']
		order.append([title, 1, leader])
...
def get_navbar(worship, language):
	...
	for item in worship['items']:
		if item['type'] == 'song':
			...
			espitems.append([ndi, "medley", "Popurr�"])
		elif item['type'] == 'prayer':
			engitems.append([ndi, "prayer", "Prayer"])
			espitems.append([ndi, "prayer", "Oraci�n"])
		elif item['type'] == 'reading':
			...
			espitems.append([ndi, "invitation", "Invitaci�n", 11])
		elif item['type'] == 'welcome':
			...
			espitems.append([ndi, "supper", "Cena del Se�or", 11])
		elif item['type'] == 'collection':
