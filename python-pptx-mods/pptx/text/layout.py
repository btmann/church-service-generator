...
class TextFitter(tuple):
    ...
    def __new__(cls, line_source, extents, font_file, features, spacing):
        width, height = extents
        return tuple.__new__(cls, (line_source, width, height, font_file, features, spacing))

    @classmethod
    def best_fit_font_size(cls, text, extents, max_size, font_file, features, spacing):
        """
        ...
        *max_size* that allows *text* to fit completely within *extents* when
        ...
        text_fitter = cls(line_source, extents, font_file, features, spacing)	# drw: added features
    ...
    def _fits_in_width_predicate(self, point_size):
        ...
        def predicate(line):
            ...
            *point_size*.
            """
            cx = _rendered_size(line.text, point_size, self._font_file, self._features)[0]
...
#            cy = _rendered_size("Ty", point_size, self._font_file, self._features)[1]
            cy = _rendered_height(point_size, self._font_file, self._features, self._spacing)	# drw
...
def _rendered_size(text, point_size, font_file, features):
    """
...
#   print(font_file, point_size, text, px_width, px_height)
#    print(font_file, point_size, text, px_width, px_height, font.getmetrics())

    return emu_width, emu_height

#
def _rendered_height(point_size, font_file, features, spacing):
    """
    Return a (width, height) pair representing the size of *text* in English
    Metric Units (EMU) when rendered at *point_size* in the font defined in
    *font_file*.
    ...
    px_height = metrics[0] + metrics[1] + spacing	# drw five for spacing

    emu_height = int(px_height / px_per_inch * emu_per_inch)

#    print(font_file, point_size, px_height)
